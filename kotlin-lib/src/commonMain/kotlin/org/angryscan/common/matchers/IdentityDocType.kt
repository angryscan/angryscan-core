...
object IdentityDocType : IHyperMatcher, IKotlinMatcher {
    ...
    override val hyperPatterns: List<String> = listOf(
        ...
        """(?:^|\s|[\(\[\{«"'])\s*(?:[\(\[\{«"'])?вид[аеу]?\s+на\s+жительств[оаеу](?:$|[\s\)\]\}»"'\.,;:!?])""",
        """(?:^|\s|[\(\[\{«"'])\s*(?:[\(\[\{«"'])?ВНЖ(?:$|[\s\)\]\}»"'\.,;:!?])""",
        """(?:^|\s|[\(\[\{«"'])\s*(?:[\(\[\{«"'])?разрешени[еяум]\s+на\s+временно[ея]\s+проживани[еяу](?:$|[\s\)\]\}»"'\.,;:!?])""",
        """(?:^|\s|[\(\[\{«"'])\s*(?:[\(\[\{«"'])?РВП(?:$|[\s\)\]\}»"'\.,;:!?])""",
        """(?:^|\s|[\(\[\{«"'])\s*(?:[\(\[\{«"'])?военн[ыойемя][йгемя]?\s+билет[аеуом]?(?:$|[\s\)\]\}»"'\.,;:!?])""",
        """(?:^|\s|[\(\[\{«"'])\s*(?:[\(\[\{«"'])?удостоверени[еяум]\s+личност[ия]\s+(?:офицер[ауом]|военнослужащего|моряк[ауом]|пенсионер[ауом])(?:$|[\s\)\]\}»"'\.,;:!?])""",
        """(?:^|\s|[\(\[\{«"'])\s*(?:[\(\[\{«"'])?национальн[ыойемя][йгемя]?\s+паспорт[аеуом]?(?:$|[\s\)\]\}»"'\.,;:!?])"""
    )
