 ...
 */
internal class RINTest {

    @Test
    fun testRINAtStart() {
        val text = " 110101199001011234 китайский ID"
        assertTrue(scanText(text, RIN) >= 1, "RIN в начале должен быть найден")
    }
    ...
    fun testRINInMiddle() {
        ...
        assertTrue(scanText(text, RIN) >= 1, "Китайский TIN в середине должен быть найден")
    }

    @Test
    fun testRINStandalone() {
        val text = " 110101199001011234 "
        assertTrue(scanText(text, RIN) >= 1, "Китайский TIN отдельно должен быть найден")
    }

    @Test
    fun testRIN18Digits() {
        val text = " 110101199001011234 "
        assertEquals(1, scanText(text, RIN), "Китайский TIN (18 цифр) должен быть найден")
    }

    @Test
    fun testRINWithX() {
        val text = " 11010119900101123X "
        assertEquals(1, scanText(text, RIN), "Китайский TIN с X должен быть найден")
    }
    ...
    fun testMultipleRINs() {
        ...
        """.trimIndent()
        assertTrue(scanText(text, RIN) >= 3, "Несколько китайских TIN должны быть найдены")
    }

    @Test
    fun testRINInvalidDate() {
        val text = " 110101199913011234 "
        assertEquals(0, scanText(text, RIN), "Китайский TIN с некорректной датой не должен быть найден")
    }

    @Test
    fun testRINInvalidMonth() {
        val text = " 110101199000011234 "
        assertEquals(0, scanText(text, RIN), "Китайский TIN с некорректным месяцем не должен быть найден")
    }

    @Test
    fun testRINInvalidDay() {
        val text = " 110101199001321234 "
        assertEquals(0, scanText(text, RIN), "Китайский TIN с некорректным днем не должен быть найден")
    }

    @Test
    fun testRINInvalidChecksum() {
        val text = " 110101199001011235 "
        assertEquals(0, scanText(text, RIN), "Китайский TIN с неверной контрольной суммой не должен быть найден")
    }

    @Test
    fun testRINEmptyString() {
        val text = ""
        assertEquals(0, scanText(text, RIN), "Пустая строка не должна содержать китайского TIN")
    }
