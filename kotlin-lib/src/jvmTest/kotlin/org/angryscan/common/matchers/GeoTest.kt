...
internal class GeoTest {
    ...
    fun testGeoZeroCoordinates() {
        val text = "0.0, 0.0"
        assertEquals(0, scanText(text, Geo), "Нулевые координаты не должны быть найдены")
    }
    ...
    fun testGeoInBrackets() {
        ...
        assertEquals(1, scanText(text, Geo), "Координаты в скобках должны быть найдены (квадратные скобки исключаются)")
    }
    ...
    fun testGeoMinimalDecimal() {
        ...
        assertEquals(1, scanText(text, Geo), "Минимальные десятичные координаты с >= 3 знаками должны быть найдены")
    }

    @Test
    fun testGeoLessThanThreeDecimals() {
        val text = "55.75, 37.61"
        assertEquals(0, scanText(text, Geo), "Координаты с < 3 знаками после запятой без метки не должны быть найдены")
    }

    @Test
    fun testGeoVerySmallValues() {
        val text = "0.048, 0.158"
        assertEquals(0, scanText(text, Geo), "Очень маленькие координаты (< 1.0) не должны быть найдены")
    }

    @Test
    fun testGeoMathematicalNotation() {
        val text = "λ~N(1.856, 0.047)"
        assertEquals(0, scanText(text, Geo), "Математические обозначения не должны быть найдены")
    }

    @Test
    fun testGeoStatisticalTermsInValue() {
        val text = "iter 55.755812, 37.617345 ewma"
        val kotlinEngine = KotlinEngine(listOf(Geo).filterIsInstance<IKotlinMatcher>())
    ...
    fun testGeoIntegerCoordinates() {
        val text = "55, 37"
        assertEquals(0, scanText(text, Geo), "Целые координаты без метки не должны быть найдены")
    }
