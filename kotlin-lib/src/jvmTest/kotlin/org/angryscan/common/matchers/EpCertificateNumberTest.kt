...
internal class EpCertificateNumberTest: MatcherTestBase(EpCertificateNumber) {
    ...
    fun testEpCertificateNumberInvalidWithoutSpaces() {
        ...
        assertEquals(0, scanText(text), "Номер без пробелов не должен быть найден")
    }
    ...
    fun testEpCertificateNumberMixedCase() {
        val text = " 00 11 22 33 44 55 66 77 88 99 Aa Bb Cc Dd Ee Ff "
        assertTrue(scanText(text) >= 1, "Номер в смешанном регистре должен быть найден")
    }
    ...
    fun testEpCertificateNumberWithSerialLabel() {
        ...
        assertTrue(scanText(text) >= 1, "Номер с меткой 'серийный номер' должен быть найден")
    }
    ...
    fun testEpCertificateNumberWithSerialNumberEP() {
        ...
        assertTrue(scanText(text) >= 1, "Номер с меткой 'serial number ЭП' должен быть найден")
    }
    ...
    fun testEpCertificateNumberWithKeyLabel() {
        ...
        assertTrue(scanText(text) >= 1, "Номер сертификата ключа должен быть найден")
    }
    ...
    fun testEpCertificateNumberWithUniqueLabel() {
        ...
        assertTrue(scanText(text) >= 1, "Уникальный номер сертификата должен быть найден")
    }
    ...
    fun testEpCertificateNumberWithColon() {
        ...
        assertTrue(scanText(text) >= 1, "Номер с двоеточием должен быть найден")
    }

    @Test
    fun testEpCertificateNumberGroupByFour() {
        val text = " Сертификат: 0011 2233 4455 6677 8899 AABB CCDD EEFF "
        assertTrue(scanText(text) >= 1, "Номер с группировкой по 4 символа должен быть найден")
    }
    ...
    fun testMultipleEpCertificateNumbers() {
        ...
        """.trimIndent()
        assertTrue(scanText(text) >= 3, "Несколько номеров сертификатов должны быть найдены")
    }

    @Test
    fun testEpCertificateNumberAllZeros() {
        val text = " 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 "
        assertEquals(0, scanText(text), "Номер из нулей не должен быть найден")
    }

    @Test
    fun testEpCertificateNumberAllF() {
        val text = " FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF "
        assertEquals(0, scanText(text), "Номер из одинаковых символов не должен быть найден")
    }
    ...
    fun testEpCertificateNumberInvalidWithInvalidChar() {
        ...
        assertEquals(0, scanText(text), "Номер с некорректным символом не должен быть найден")
    }

    @Test
    fun testEpCertificateNumberEmptyString() {
        val text = ""
        assertEquals(0, scanText(text), "Пустая строка не должна содержать номера сертификата ЭП")
    }
}
