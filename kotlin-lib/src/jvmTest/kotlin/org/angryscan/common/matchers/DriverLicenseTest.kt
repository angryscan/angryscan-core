...
internal class DriverLicenseTest: MatcherTestBase(DriverLicense) {
    ...
    fun testDriverLicenseAtStart() {
        ...
        assertTrue(scanText(text) >= 1, "ВУ в начале должно быть найдено")
    }

    @Test
    fun testDriverLicenseAtEnd() {
        val text = "Номер водительского удостоверения: 77 12 123456 "
        assertTrue(scanText(text) >= 1, "ВУ в конце должно быть найдено")
    }

    @Test
    fun testDriverLicenseInMiddle() {
        val text = "Водитель с ВУ 77 12 123456 зарегистрирован"
        assertTrue(scanText(text) >= 1, "ВУ в середине должно быть найдено")
    }
    ...
    fun testDriverLicenseWithSpaces() {
        val text = " 77 12 123456 "
        assertTrue(scanText(text) >= 1, "ВУ с пробелами должно быть найдено")
    }

    @Test
    fun testDriverLicenseWithoutSpaces() {
        val text = " 7712 123456 "
        assertTrue(scanText(text) >= 1, "ВУ без первого пробела должно быть найдено")
    }

    @Test
    fun testDriverLicenseWithSpaceInSeries() {
        val text = " 77 12 123456 "
        assertTrue(scanText(text) >= 1, "ВУ с пробелом в серии должно быть найдено")
    }

    @Test
    fun testDriverLicenseWithoutSpaceInSeries() {
        val text = " 7712 123456 "
        assertTrue(scanText(text) >= 1, "ВУ без пробела в серии должно быть найдено")
    }

    @Test
    fun testDriverLicenseWithPrefix() {
        val text = "водительское удостоверение: 77 12 123456 "
        assertTrue(scanText(text) >= 1, "ВУ с префиксом должно быть найдено")
    }

    @Test
    fun testDriverLicenseWithAbbreviation() {
        val text = "ВУ: 77 12 123456 "
        assertTrue(scanText(text) >= 1, "ВУ с аббревиатурой должно быть найдено")
    }

    @Test
    fun testDriverLicenseBoundary00() {
        val text = " 00 00 000000 "
        assertTrue(scanText(text) >= 1, "ВУ с нулями должно быть найдено")
    }

    @Test
    fun testDriverLicenseBoundary99() {
        val text = " 99 99 999999 "
        assertTrue(scanText(text) >= 1, "ВУ с девятками должно быть найдено")
    }

    @Test
    fun testDriverLicenseMoscow() {
        val text = " 77 12 123456 "
        assertTrue(scanText(text) >= 1, "Московское ВУ должно быть найдено")
    }

    @Test
    fun testDriverLicenseSPb() {
        val text = " 78 12 123456 "
        assertTrue(scanText(text) >= 1, "Петербургское ВУ должно быть найдено")
    }

    @Test
    fun testDriverLicenseInParentheses() {
        val text = "(77 12 123456) "
        assertTrue(scanText(text) >= 1, "ВУ в скобках должно быть найдено")
    }

    @Test
    fun testDriverLicenseInQuotes() {
        val text = "\"77 12 123456\" "
        assertTrue(scanText(text) >= 1, "ВУ в кавычках должно быть найдено")
    }

    @Test
    fun testDriverLicenseWithPunctuation() {
        val text = "ВУ: 77 12 123456. "
        assertTrue(scanText(text) >= 1, "ВУ с точкой должно быть найдено")
    }

    @Test
    fun testDriverLicenseUpperCase() {
        val text = "ВОДИТЕЛЬСКОЕ УДОСТОВЕРЕНИЕ: 77 12 123456 "
        assertTrue(scanText(text) >= 1, "ВУ в верхнем регистре должно быть найдено")
    }

    @Test
    fun testDriverLicenseLowerCase() {
        val text = "водительское удостоверение: 77 12 123456 "
        assertTrue(scanText(text) >= 1, "ВУ в нижнем регистре должно быть найдено")
    }
    ...
    fun testMultipleDriverLicenses() {
        val text = """
            ...
            Третье: 50 56 789012
        """.trimIndent()
        assertTrue(scanText(text) >= 3, "Несколько ВУ должны быть найдены")
    }

    @Test
    fun testDriverLicenseTooShort() {
        val text = "77 12 12345"
        assertEquals(0, scanText(text), "Слишком короткое ВУ не должно быть найдено")
    }

    @Test
    fun testDriverLicenseTooLong() {
        val text = "77 12 1234567"
        assertEquals(0, scanText(text), "Слишком длинное ВУ не должно быть найдено")
    }

    @Test
    fun testDriverLicenseWithLetters() {
        val text = "77 AB 123456"
        assertEquals(0, scanText(text), "ВУ с буквами не должно быть найдено")
    }
