...
class AddressTest: MatcherTestBase(Address) {
    ...
    fun testAddressWithGor() {
        ...
        assertTrue(scanText(text) >= 1, "Адрес с 'гор.' должен быть найден")
    }
    ...
    fun testAddressWithDom() {
        ...
        assertTrue(scanText(text) >= 1, "Адрес с 'дом' должен быть найден")
    }

    @Test
    fun testAddressShort() {
        val text = "г. Москва, дом 5"
        assertTrue(scanText(text) >= 1, "Короткий адрес должен быть найден")
    }

    @Test
    fun testAddressLong() {
        val text = "г. Москва, ул. Ленина, д. 10, корп. 2, стр. 3, кв. 45"
        assertTrue(scanText(text) >= 1, "Длинный адрес должен быть найден")
    }

    @Test
    fun testAddressWithKorpus() {
        val text = "г. Москва, ул. Ленина, д. 10, корп. 2"
        assertTrue(scanText(text) >= 1, "Адрес с корпусом должен быть найден")
    }
    ...
    fun testAddressUpperCase() {
        val text = "Г. МОСКВА, УЛ. ЛЕНИНА, Д. 10"
        assertTrue(scanText(text) >= 1, "Адрес в верхнем регистре должен быть найден")
    }

    @Test
    fun testAddressLowerCase() {
        val text = "г. москва, ул. ленина, д. 10"
        assertTrue(scanText(text) >= 1, "Адрес в нижнем регистре должен быть найден")
    }

    @Test
    fun testAddressMixedCase() {
        val text = "Г. МоСкВа, Ул. ЛеНиНа, Д. 10"
        assertTrue(scanText(text) >= 1, "Адрес в смешанном регистре должен быть найден")
    }
    ...
    fun testAddressWithoutD() {
        ...
        assertEquals(0, scanText(text), "Адрес без 'д.' не должен быть найден")
    }
